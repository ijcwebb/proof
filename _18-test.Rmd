...
- He used $n$ instead of $a$. It's OK; it's just a different letter.
- He uses the mathematical symbol $\forall$, which means "for all". For example "AUCA studdent ID numbers are 4-digits long $\forall$ students" or "$a^2$ is even $\forall$ even integers $a$".
- At one point he introduces $k$. Here, $k$ is simply a specific value of $n$.
- At another point he write $22^k$ but he means $2 \cdot 2^k$ or $2 \times 2^k$. It's clear from the context what he meant.
...
**Step 2:** We should show that the conjecture is true for the base case. As the conjecture concerns non-negative integers, our base case is $a_0 = 0$. 0 is even, because $0 = 2(0)$. We are done.
```
</div>
